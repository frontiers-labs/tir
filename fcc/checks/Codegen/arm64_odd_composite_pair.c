...
// CHECK-LABEL: %{{[0-9]+}} = func.func @consume_twelve(%{{[0-9]+}}: !tuple<!i64, !i64>) -> !i64 {
// CHECK: ptr.alloca {size = 16, align = 8}
// CHECK-LABEL: %{{[0-9]+}} = func.func @call_twelve(
// CHECK: ptr.alloca {size = 16, align = 8}
// CHECK-COUNT-2: constant {value = 0} : !i64
// CHECK: constant {value = 12} : !i64
// CHECK: ptr.memcpy
// CHECK: make_tuple {{.*}} : !tuple<!i64, !i64>
// CHECK: func.call %{{[0-9]+}}({{.*}} : !tuple<!i64, !i64>) -> !i64
// CHECK-LABEL: %{{[0-9]+}} = func.func @make_twelve(
