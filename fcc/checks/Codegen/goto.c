...
// CHECK: state(%{{[0-9]+}}, %[[OUT:[0-9]+]], %{{[0-9]+}}) = scf.loop (state(%{{[0-9]+}} = %{{[0-9]+}}, %{{[0-9]+}} = %{{[0-9]+}}, %{{[0-9]+}} = %{{[0-9]+}})) {
// CHECK: %{{[0-9]+}}, state(%{{[0-9]+}}) = ptr.load %{{[0-9]+}} state(%{{[0-9]+}}) : !i32
// CHECK-NEXT: %{{[0-9]+}}, state(%[[E:[0-9]+]]) = ptr.load %{{[0-9]+}} state(%{{[0-9]+}}) : !i32
// CHECK: %[[P:[0-9]+]], state(%[[D0:[0-9]+]], %[[D1:[0-9]+]]) = scf.switch
// CHECK: -> %[[P]], state(%[[E]], %[[D0]], %[[D1]], %[[E]], %[[D0]], %[[D1]])
// CHECK: }
