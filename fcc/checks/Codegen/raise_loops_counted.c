...
// A counted `for` is raised to `scf.for2`: the counter leaves its slot for a
...
// CHECK: %{{[0-9]+}}, %[[FINAL:[0-9]+]] | %[[OUT:[0-9]+]] = scf.for2 %[[IV:[0-9]+]] = %[[LB]] to %[[UB]] step %[[ST:[0-9]+]] (%[[ARG:[0-9]+]] = %[[LB]] | %[[DEP:[0-9]+]] = %{{[0-9]+}}) {
