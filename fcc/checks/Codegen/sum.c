...
// CHECK: %[[C0:[0-9]+]], %[[C1:[0-9]+]], %{{[0-9]+}} = state.split state(%[[E]])
// CHECK: %[[S0:[0-9]+]] = ptr.store %{{[0-9]+}}, %{{[0-9]+}} state(%[[C0]])
// CHECK: %[[S1:[0-9]+]] = ptr.store %{{[0-9]+}}, %{{[0-9]+}} state(%[[C1]])
// CHECK: %[[A:[0-9]+]], %{{[0-9]+}} = ptr.load %{{[0-9]+}} state(%[[S0]]) : !i32
// CHECK: %[[B:[0-9]+]], %{{[0-9]+}} = ptr.load %{{[0-9]+}} state(%[[S1]]) : !i32
// CHECK: addi %[[A]], %[[B]] : !i32
// CHECK: -> %{{[0-9]+}}, %{{[0-9]+}}
