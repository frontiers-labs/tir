...
// CHECK: state(%[[CLEAR:[0-9]+]]) = ptr.store %[[ZERO]], %[[SLOT:[0-9]+]]
// CHECK-NEXT: state(%[[COPY:[0-9]+]]) = ptr.memcpy %[[SLOT]], %{{[0-9]+}}, %[[SIZE]] state(%[[CLEAR]])
// CHECK-NEXT: %[[WORD:[0-9]+]], state(%[[LOAD:[0-9]+]]) = ptr.load %[[SLOT]] state(%[[COPY]]) : !i64
// CHECK-NEXT: %{{[0-9]+}}, state(%{{[0-9]+}}) = func.call %{{[0-9]+}}(%[[WORD]] : !i64) -> !i64 state(%[[LOAD]])
// CHECK-LABEL: %{{[0-9]+}} = func.func @make_three(
// CHECK-SAME: ) -> !i64 {
// CHECK: ptr.alloca {size = 8, align = 8}
// CHECK: ptr.memcpy
// CHECK: state(%[[RET_COPY:[0-9]+]]) = ptr.memcpy %[[RET:[0-9]+]]
// CHECK-NEXT: %[[RET_WORD:[0-9]+]], state(%[[RET_LOAD:[0-9]+]]) = ptr.load %[[RET]] state(%[[RET_COPY]]) : !i64
// CHECK-NEXT: state(%[[RET_OUT:[0-9]+]]) = state.join state(%{{[0-9]+}}, %[[RET_LOAD]])
// CHECK-NEXT: -> %[[RET_WORD]], state(%[[RET_OUT]])
