...
// CHECK-NEXT: %[[PCHAIN:[0-9]+]], %[[CCHAIN:[0-9]+]], %{{[0-9]+}} = state.split state(%[[E]])
// CHECK-NEXT: %[[PARAM:[0-9]+]] = ptr.store %{{[0-9]+}}, %[[PSLOT:[0-9]+]] state(%[[PCHAIN]])
// CHECK-NEXT: %[[INIT:[0-9]+]] = ptr.store %{{[0-9]+}}, %[[CSLOT:[0-9]+]] state(%[[CCHAIN]])
// CHECK-NEXT: %[[LB:[0-9]+]], %{{[0-9]+}} = ptr.load %[[CSLOT]] state(%[[INIT]]) : !i32
// CHECK-NEXT: %[[UB:[0-9]+]], %{{[0-9]+}} = ptr.load %[[PSLOT]] state(%[[PARAM]]) : !i32
