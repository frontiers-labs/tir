...
// CHECK-NEXT: state(%[[MAKE_OUT:[0-9]+]]) = state.join state(%{{[0-9]+}}, %{{[0-9]+}}, %{{[0-9]+}}, %[[MAKE_COPY]])
// CHECK-NEXT: -> %[[MAKE_DEST]], state(%[[MAKE_OUT]])
// CHECK-LABEL: %{{[0-9]+}} = func.func @forward_large(%[[FORWARD_DEST:[0-9]+]]: !ptr.p,
// CHECK-SAME: ) -> !ptr.p result_address {
// CHECK: %[[TEMP:[0-9]+]] = ptr.alloca {size = 24, align = 8}
// CHECK: %{{[0-9]+}}, state(%[[CALL:[0-9]+]]) = func.call %{{[0-9]+}}(%[[TEMP]]
// CHECK-SAME: ) -> !ptr.p result_address
// CHECK: state(%[[FORWARD_COPY:[0-9]+]]) = ptr.memcpy %[[FORWARD_DEST]], %[[TEMP]], %{{[0-9]+}} state(%[[CALL]])
// CHECK-NEXT: state(%[[FORWARD_OUT:[0-9]+]]) = state.join state(%{{[0-9]+}}, %{{[0-9]+}}, %{{[0-9]+}}, %[[FORWARD_COPY]])
// CHECK-NEXT: -> %[[FORWARD_DEST]], state(%[[FORWARD_OUT]])
