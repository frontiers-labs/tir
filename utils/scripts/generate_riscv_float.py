...
def behavior(expression, destination, source_width, destination_width, mnemonic):
    value = expression.replace("ROUND", "$($round)*")
    result = "value"
    if destination == "rd":
...
def rounded_variants():
    ...
    for suffix, rm, text in MODES:
        name = "${concat($name, " + suffix + ")}" if suffix else "$name"
        opname = '${concat($mnemonic, ".' + text + '")}' if text else "$mnemonic"
        assembly = '${concat($assembly, ", ' + text + '")}' if text else '($assembly, ${concat($assembly, ", dyn")})'
        body = f"$body!(0b{rm:03b})"
        if rm == 7:
            body = "if FRM::frm ~> 0b100 { trap(2); } else { $body!(FRM::frm) }"
        variants.append(
            f"        instruction {name} for [$feature] : $tmpl {{\n"
...
def generate():
    ...
    for (
        ...
        body = behavior(expression, destination, source_width, destination_width, mnemonic)
        defs.append(
            f"macro {name}Body {{\n"
            "    ($($round:tt)*) => {\n"
            f"{body}\n    }};\n}}\n"
        ...
        defs.append(
            f'rounded_variants!({name}, "{mnemonic}", {extension}, {name}Rounded, '
            f'{name}Body, "{{self.MNEMONIC}} {names}")\n'
        )
        for suffix, rm, text in MODES:
